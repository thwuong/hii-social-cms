...
const KPICard = ({ 
  ...
  >
    ...
    <div className="absolute top-0 left-0 w-full h-[1px] bg-white transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
  ...
  return (
    <div className="animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-20 gap-8">
        <div>
            <div className="font-mono text-[10px] text-zinc-500 uppercase tracking-[0.3em] mb-4">KHÔNG GIAN LÀM VIỆC ALPHA</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[2px] bg-white/10 border border-white/10 p-[1px]">
             ...
             <div className="h-[200px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area type="monotone" dataKey="count" stroke="#fff" strokeWidth={1} fillOpacity={1} fill="url(#colorCount)" />
                  </AreaChart>
                </ResponsiveContainer>
             </div>
         </div>
         ...
         {/* KPI Grid */}
         <KPICard count={kpiData.draft} subtext="NHÁP" onClick={() => onNavigate({ status: ContentStatus.DRAFT })} />
         <KPICard count={kpiData.approved} subtext="ĐÃ DUYỆT" onClick={() => onNavigate({ status: ContentStatus.APPROVED })} />
         <KPICard count={kpiData.published} subtext="ĐÃ ĐĂNG" onClick={() => onNavigate({ status: ContentStatus.PUBLISHED })} />
         <KPICard count={kpiData.rejected} subtext="TỪ CHỐI" onClick={() => onNavigate({ status: ContentStatus.REJECTED })} />
         <KPICard count={kpiData.scheduled} subtext="ĐÃ LÊN LỊCH" onClick={() => onNavigate({ status: ContentStatus.SCHEDULED })} />
